...
use_js_utils <- function(overwrite = FALSE, quiet = FALSE) {
  ...
  if (any(grepl("leprechaun-utils.js", index))) return(invisible(FALSE))
...
use_html_utils <- function(overwrite = FALSE, quiet = FALSE) {
  ...
  if (!quiet) cli_alert_success("Creating {.file R/html-utils.R}")
...
use_endpoints_utils <- function(overwrite = FALSE, quiet = FALSE) {
  ...
  if (!quiet) cli_alert_success("Creating {.file R/endpoint-utils.R}")
...
use_packer <- function(quiet = FALSE) {
  ...
  if (!quiet) cli_alert_success("Creating {.file inst/dev/packer.R}")
...
check_config <- function() {
  if (!file_exists("inst/config.yml")) return()
...
has_build <- function(which) {
  if (missing(which)) stop("Missing `which`", call. = FALSE)
