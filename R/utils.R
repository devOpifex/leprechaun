...
tmp_replace <- function(content, module = NULL, bs_version = NULL) {
  ...
  if (!is.null(module)) content <- gsub("#MODULE#", module, content)

  if (!is.null(bs_version)) content <- gsub("#BSVERSION#", bs_version, content)
...
#' @noRd
#' @keywords internal
tmp_read_replace_write <- function(
  infile,
  outfile,
  module = NULL,
  bs_version = NULL
...
get_html_utils_file <- function() {
  ...
  if (bs_version == 4) return("utils_v4.R")
